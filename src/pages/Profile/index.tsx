                    ...
                    loading ? <LoadingAtom/> :
                        <>
                            ...
                            <Options>{!own &&<ButtomAtom onClick={handleFollowProfile} type={"button"} size={"l"}
                                                 stetic={"soft"}>{!!profile.user_follow?.find(z => z.profile.id == contextProfile.id) ? "unfollow" : "follow"}</ButtomAtom>}</Options>
                            </BackGround>
                            <UserData>
                            ...
                            <UserPosts>
                                {profilePosts.sort((a, b) => new Date(a.createdAt as string) < new Date(b.createdAt as string) ? 1 : -1).map(e => (
